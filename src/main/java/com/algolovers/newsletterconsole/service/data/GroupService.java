    ...
    public Result<Invitation> inviteUserToGroup(@Valid GroupUserInvitationRequest groupUserInvitationRequest, User authenticatedUser) {
        try {
            ...
            if (Objects.isNull(invitedUser)) {
                emailService.sendInvitationEmail(authenticatedUser, groupUserInvitationRequest.getUserEmail(), "register an account in Newsletter", "register an account. Proceed by visiting Console > Invitations and look for \"%s\"".formatted(group.getGroupName()));
                return new Result<>(true, invitation, "Invitation created, user does not exist. An invitation to the app has been sent");
            } else {
                emailService.sendInvitationEmail(authenticatedUser, groupUserInvitationRequest.getUserEmail(), "join %s".formatted(group.getGroupName()), "open newsletter. Proceed by visiting Console > Invitations and look for \"%s\"".formatted(group.getGroupName()));
